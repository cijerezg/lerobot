...
- `lerobot.rl.inference_pi05_async` is the equivalent of `lerobot-record` for inference. It works on the real robot and it runs RTC asynchronously at 30Hz. It keeps an online buffer with episode data that is periodically saved. Every few episodes, it also generates the following video:

<div align="center">
  <img src="media/readme/episode_video.webp" width="100%" alt="Episode video">
  <p><em>Inference video with subtasks generated by VLA and critic values.</em></p>
</div>

The video is also generated in the learner pipeline.
...
2. **π<sub>0.5</sub> base weights** — downloaded locally from Hugging Face. Set the path in `policy.pi05_checkpoint`.
...
| `policy.pi05_checkpoint` | Path to your π<sub>0.5</sub> base weights (or offline checkpoint) |
...
**Option 1 — Automatic (LLM-based):** The `subtask_annotate.py` script uses Qwen2-VL or Qwen3-VL to segment episodes into skills by watching the video. In practice, the Qwen models produced noisy annotations that needed significant cleanup — it's an option, but don't expect it to be hands-off.
...
The complication with anchor encoding is that leftover actions reference a stale anchor s<sub>0</sub>. `align_prev_actions` in `actor_pi05_async_utils.py` corrects this in absolute space, then re-normalizes. What makes it non-trivial is per-timestep normalization: stats have shape `[chunk_size, action_dim]`, so each position in the chunk has its own mean/std. You can't just shift normalized values directly — the function right-aligns the leftover into a padded buffer before unnormalizing (so each value uses the stats for its original chunk position), applies the anchor delta, then re-normalizes left-aligned for the next chunk. For delta encoding with offset > 0, no correction is needed since leftover actions are consecutive diffs that don't reference s<sub>0</sub>.
