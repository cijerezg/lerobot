...
if TYPE_CHECKING or _transformers_available:
    ...
    try:
        ...
        # Fallback for transformers < 5.x where the scaled word embedding
        # class has not yet been split out. Behaviour matches the v5
        # implementation: embeddings are scaled by ``embed_scale`` at
        # forward time without introducing additional learnable params.
        class GemmaTextScaledWordEmbedding(nn.Embedding):
            ...
            def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
                ...
                return out * self.embed_scale.to(device=out.device, dtype=out.dtype)
...
class PiStar06Policy(PreTrainedPolicy):
    ...
    def _preprocess_images(
        ...
        if target_dtype not in (torch.float32, torch.bfloat16, torch.float16):
    ...
    def _setup_advantage_tokens(self) -> None:
        ...
        pos_ids = tokenizer.encode(
            " Advantage: positive good great successful correct excellent optimal precise accurate effective superior favorable improved desirable",
            add_special_tokens=False,
        )
        neg_ids = tokenizer.encode(
            " Advantage: negative bad poor failed incorrect wrong suboptimal imprecise inaccurate ineffective inferior unfavorable degraded undesirable",
            add_special_tokens=False,
        )
    ...
    def _setup_episode_metadata(
        ...
        if episodes is None:
            logging.warning(
                "dataset_meta.episodes is None (empty/new dataset); "
                "skipping episode metadata setup"
            )
            return
