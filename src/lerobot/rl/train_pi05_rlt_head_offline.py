...
@dataclass
class TrainPI05RLTHeadOfflineConfig:
    policy_path: str = "outputs/pi05_subtasks_good_dataset_4/checkpoints/last/pretrained_model"
    replay_buffer_path: str = "outputs/rlt_online/rlt_online_replay.pt"
    output_dir: str = "outputs/rlt_offline_head"
    device: str = "cuda"
    seed: int = 0
...
def _build_policy(cfg: TrainPI05RLTHeadOfflineConfig, replay: RLTReplayBuffer) -> PI05RLTPolicy:
...
def _init_wandb(cfg: TrainPI05RLTHeadOfflineConfig):
...
def _save_head(policy, output_dir: Path, step: int, cfg: TrainPI05RLTHeadOfflineConfig) -> None:
    ...
    logging.info("Saved offline RLT head checkpoint to %s", path)


@draccus.wrap()
def train_pi05_rlt_head_offline(cfg: TrainPI05RLTHeadOfflineConfig) -> None:
    init_logging()
    _set_seed(cfg.seed)
    ...
    logging.info(
        "Starting offline RLT head training | replay=%s | samples=%d | device=%s | steps=%d",
