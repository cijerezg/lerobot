...
class PI05RLConfig(PI05FullConfig):
    ...
    critic_target_update_every: int = (
        1  # Run Polyak every N optimizer steps (batch updates with proportionally larger τ to clear BF16 ULP)
    )
    discount: float = 0.97
    ...
    # Pretrained merge (periodic convex pull toward pretrained weights, with optimizer-state reset)
    # alpha == 0 or every_n_steps == 0 disables.
    pretrained_merge_alpha: float = 0.0
    pretrained_merge_every_n_steps: int = 0
    pretrained_merge_targets: list[str] = field(default_factory=lambda: ["actor", "critic"])
...
class Pi05TransformerCritic(nn.Module):
    ...
    def __init__(self, config: PI05RLConfig):
        ...
        self.layers = nn.ModuleList(
            [pi_gemma_decoder_layer_base(critic_gemma_config, layer_idx=i) for i in range(num_layers)]
        )
    ...
    def hl_gauss_target(self, target_v: Tensor) -> Tensor:
        ...
        z = (internal_edges.unsqueeze(0) - target_v.unsqueeze(-1)) / (self.hl_gauss_sigma * math.sqrt(2))
...
class PI05RLPytorch(PI05Pytorch):
    ...
    def forward(
        ...
        combined_att_2d_masks = torch.zeros(
            bsize, total_len, total_len, dtype=torch.bool, device=prefix_embs.device
        )
        ...
        )  # Ensure float32 if proj is float32
        ...
        if self.log_counter % 240 == 0 and not self.suppress_debug_log:
            print(
                f"[Actor Loss] Total: {total_loss.item():.4f} | Flow: {flow_loss.mean().item():.4f} | ActionCE: {action_ce_loss.item():.4f} | SubtaskCE: {subtask_ce_loss.item():.4f}"
            )
            if subtask_tokens is not None:
                pred_ids = subtask_logits.argmax(dim=-1)  # [B, subtask_len]
                for i in range(min(2, pred_ids.shape[0])):
                    tgt = self._paligemma_tokenizer.decode(
                        subtask_labels[i], skip_special_tokens=True
                    ).strip()
...
class PI05RLPolicy(PI05FullPolicy):
    ...
    def __init__(self, config: PI05RLConfig):
        ...
        if config.base_path:
            ...
            if state_dict is not None:
                ...
                if any("critic" in k for k in state_dict.keys()) or any(
                    "advantage_mlp" in k for k in state_dict.keys()
                ):
            ...
            if is_rl_checkpoint:
                ...
                if hasattr(self, "critic"):
                    ...
                    missing_critic, unexpected_critic = self.critic.load_state_dict(
                        critic_state_dict, strict=False
                    )
                    print(
                ...
                missing_keys, unexpected_keys = self.model.load_state_dict(
                    temp_policy.model.state_dict(), strict=False
                )
    ...
    def get_vision_features(
        self, batch: dict[str, Tensor], use_grad: bool = True, encoder_module=None
    ) -> tuple[Tensor, Tensor]:
    ...
    def forward(
        ...
        if model != "actor":
            vision_features, vision_pad_masks = self.get_vision_features(
                batch, use_grad=use_grad, encoder_module=self.critic
            )
        ...
        if model == "actor":
            ...
            with torch.no_grad():
                ...
                if "next_state" in batch:
                    ...
                    actor_embed_layer = (
                        self.model.paligemma_with_expert.paligemma.model.language_model.embed_tokens
                    )
                    next_critic_text_embs = actor_embed_layer(next_critic_tokens).detach()

                    next_v = self.critic_target(
                        next_vision_features, next_critic_text_embs, next_critic_token_masks
                    )["value"]
                else:
            ...
            return {"critic_values": values, "critic_value_mean": values.mean().item()}
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor], **kwargs) -> Tensor:
        ...
        if should_regenerate:
            subtask_tokens, subtask_masks = self.model.generate_subtask_tokens(
                images, img_masks, tokens, masks, max_decoding_steps=self.config.max_decoding_steps
            )
