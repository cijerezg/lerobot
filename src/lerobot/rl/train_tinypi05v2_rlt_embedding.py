...
"""Train the RL-token autoencoder on top of a frozen tinypi05v2 checkpoint.

This is the tinypi05v2 analog of `train_tinypi05_rlt_embedding.py`. The frozen
tinypi05v2 backbone produces prefix embeddings; the autoencoder is trained to
...
class TrainTinyPI05V2RLTEmbeddingConfig:
    ...
    # Leave None to default to vlm_width (recommended for tinypi05v2).
...
def _build_policy(cfg: TrainTinyPI05V2RLTEmbeddingConfig) -> TinyPI05V2RLTPolicy:
...
def _build_dataset(cfg: TrainTinyPI05V2RLTEmbeddingConfig, policy: TinyPI05V2RLTPolicy):
...
def _prepare_batch(raw_batch: dict, cfg: TrainTinyPI05V2RLTEmbeddingConfig, device: str) -> dict:
...
def _save_checkpoint(
    ...
    output_dir: Path,
    step: int,
    cfg: TrainTinyPI05V2RLTEmbeddingConfig,
) -> None:
...
@draccus.wrap()
def train_tinypi05v2_rlt_embedding(cfg: TrainTinyPI05V2RLTEmbeddingConfig) -> None:
    init_logging()
    ...
    # Use the saved tinypi05v2 processor pipeline from the checkpoint so the
...
if __name__ == "__main__":
    train_tinypi05v2_rlt_embedding()
