...
class SharedStateVLA:
    """Thread-safe state bag passed between the inference and env threads."""
    ...
    def __init__(self, chunk_size: int):
        ...
        self.params_loaded_event.set()  # set so first episode doesn't block forever

        # Action chunk double-buffer
        self._action_deque: deque = deque()
        self.chunk_request_event = threading.Event()  # env → inference: "need new chunk"
        self.chunk_ready_event = threading.Event()    # inference → env: "chunk ready"
    ...
    def get_latest_observation(self) -> dict | None:
        with self.lock:
            return {k: v for k, v in self.latest_obs.items()} if self.latest_obs else None
...
def pull_new_policy_weights_vla(
    policy: nn.Module,
    parameters_queue,
    device: torch.device,
) -> None:
    """Drain the latest weight packet from the queue and load into policy."""
    blob = get_last_item_from_queue(parameters_queue, block=False)
    if blob is None:
        return
    logger.info("[ACTOR] Loading new parameters from Learner.")
    state_dicts = bytes_to_state_dict(blob)
    sd = move_state_dict_to_device(state_dicts["policy"], device=device)
    policy.load_state_dict(sd, strict=False)
    logger.info("[ACTOR] Parameters loaded.")


# ── Inference thread ──────────────────────────────────────────────────────────


def get_actions_worker_vla(
    policy: nn.Module,
    preprocessor,
    postprocessor,
    shared_state: SharedStateVLA,
    parameters_queue,
    device: torch.device,
    cfg,
) -> None:
    ...
    Waits for the env thread to request a new chunk, generates it, and signals
    the env thread that the chunk is ready.  Weight updates are applied between
    chunks so they never interrupt env execution.
    """
    try:
        ...
        while shared_state.running:
            # -- pull weight update if env thread requested one ---------------
            if shared_state.check_and_clear_parameter_update():
                pull_new_policy_weights_vla(policy, parameters_queue, device)
                shared_state.params_loaded_event.set()

            # -- wait for env thread to need a new chunk ----------------------
            ...
            # -- fetch latest obs and preprocess ------------------------------
            obs = shared_state.get_latest_observation()
            if obs is None:
                shared_state.chunk_ready_event.set()  # unblock env even if empty
                continue

            obs_filtered = {k: v for k, v in obs.items() if k in cfg.policy.input_features}
            batch = preprocessor({**obs_filtered, "task": task_str})

            # -- generate full action chunk -----------------------------------
            t0 = time.perf_counter()
            with torch.no_grad():
                # select_action caches the chunk internally; first call runs the model,
                # subsequent calls pop from the cache without re-running inference.
                actions: list[torch.Tensor] = []
                for i in range(chunk_size):
                    act = policy.select_action(batch)  # [action_dim] normalized
            ...
            logger.debug(f"[INFERENCE] Chunk generated in {dt:.3f}s")
...
def env_interaction_worker_vla(
    online_env,
    env_processor,
    action_processor,
    shared_state: SharedStateVLA,
    ...
    Manages episode lifecycle, pulls actions from the chunk buffer (requesting
    new chunks from the inference thread as needed), collects transitions, and
    pushes completed episodes to the learner.
    """
    try:
        ...
        def _wait_for_teleop_start():
            logger.info("[ENV] Waiting for '2' on teleop device to start episode…")
            while shared_state.running:
        ...
        def _request_and_wait_chunk() -> None:
        ...
        while shared_state.running:
            if shared_state.params_loaded_event.wait(timeout=0.5):
                break
        logger.info("[ENV] Weights loaded. Starting episode.")

        # request first chunk before marking episode active
        _request_and_wait_chunk()
        ...
        while shared_state.running:
            ...
            if not shared_state.episode_active:
                logger.info("[ENV] Episode ended. Waiting for teleop '2' to start next…")
                _wait_for_teleop_start()
            ...
            # ── get action ────────────────────────────────────────────────
            if is_intervening:
                # use teleop joint positions directly
                ...
                # request new chunk if running low
            ...
            is_intervening = bool(intervention_info.get(TeleopEvents.IS_INTERVENTION, False))
            ...
            # update shared obs for inference thread
            ...
            sleep = max(0.0, action_interval - dt)
            shared_state.env_wait_time += sleep
            precise_sleep(sleep)
...
def act_with_policy_async_vla(
    ...
    logger.info("[ACTOR] Building policy…")
    policy = trainer.make_policy(cfg)
    policy = policy.to(device).eval()

    logger.info("[ACTOR] Building processors…")
    ...
    shared_state = SharedStateVLA(chunk_size=chunk_size)
    shared_state.running = not shutdown_event.is_set()

    inference_thread = Thread(
        target=get_actions_worker_vla,
        args=(policy, preprocessor, postprocessor, shared_state, parameters_queue, device, cfg),
        daemon=True,
        name="vla_inference",
    )
    env_thread = Thread(
        target=env_interaction_worker_vla,
        args=(
            online_env, env_processor, action_processor, shared_state,
    ...
    try:
        ...
        logger.info("[ACTOR] Threads running. Supervisor loop active.")
        t_start = time.time()
        while not shutdown_event.is_set():
            time.sleep(10)
            metrics = shared_state.get_and_reset_metrics()
            n = max(1, metrics["env_steps"])
            lats = metrics["inference_latencies"]
            lat_str = (
                f"avg={sum(lats)/len(lats):.3f}s  max={max(lats):.3f}s"
                if lats else "N/A"
            )
            logger.info(
                f"[ACTOR] runtime={int(time.time()-t_start)}s  "
                f"env_steps={metrics['env_steps']}  "
                f"avg_env_active={metrics['env_active_time']/n:.4f}s  "
        ...
        shared_state.chunk_request_event.set()  # unblock inference thread

        for t in (inference_thread, env_thread):
            t.join(timeout=5.0)
        try:
            online_env.close()
        except Exception:
