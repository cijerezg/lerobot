...
def _saved_processor_path(cfg: Any) -> Path | None:
    ...
    if getattr(cfg, "output_dir", None):
        candidates.append(
            Path(cfg.output_dir).expanduser() / CHECKPOINTS_DIR / LAST_CHECKPOINT_LINK / PRETRAINED_MODEL_DIR
        )
...
class MolmoAct2Trainer(Trainer):
    ...
    _AIM_METRIC_KEYS = frozenset(
        {
            ...
            "action_aux/tempered_mse_mean",
            "action_aux/band_dc_mse_mean",
            "action_aux/band_k1_mse_mean",
            "action_aux/band_k2_mse_mean",
            "action_aux/band_k3_mse_mean",
            "action_aux/band_detail_mse_mean",
            "action_aux/band_high_mse_mean",
            "action_aux/full_chunk_active_fraction",
            "discrete_aux/ordinal_ce_mean",
            "discrete_aux/band_dc_ordinal_ce_mean",
            "discrete_aux/band_k1_ordinal_ce_mean",
            "discrete_aux/band_k2_ordinal_ce_mean",
            "discrete_aux/band_k3_ordinal_ce_mean",
            "discrete_aux/band_detail_ordinal_ce_mean",
    ...
    def _apply_actor_freeze(policy: nn.Module, tp, *, freeze_embedding: bool) -> None:
        ...
        vt_layers = set(range(tp.vision_from_layer, _MOLMOACT2_VIT_DEPTH)) if vision_on else set()
        lm_layers = set(range(tp.language_from_layer, _MOLMOACT2_LANGUAGE_DEPTH)) if language_on else set()
    ...
    def build_training_batch(
        ...
        Subtask/metadata columns from the sampler ride complementary_data so the
        prompt step renders their clauses (with train-time dropout).
        """
    ...
    def actor_forward(
        ...
        policy: nn.Module,
        batch: dict,
        advantage: torch.Tensor,  # noqa: ARG002 — unused; kept for the Trainer interface
        cfg,  # noqa: ARG002
    ) -> dict[str, Any]:
        """Forward pass."""
    ...
    def update_actor(
        ...
        **kwargs,
        ...
        """
        Actor update: sample → preprocess → forward → backward → step.
        ...
        capture_modality_telemetry = optimization_step is None or int(optimization_step) % log_freq == 0
        ...
        for accum_idx in range(grad_accum):
            ...
            # pointmap_config set).
            observations = self._inject_depth_observations(observations, raw.get("complementary_info"), cfg)
            # One-shot depth check for the TRAINING path (the inference path has its own):
            ...
            if pointmap_config is not None and not getattr(self, "_depth_training_logged", False):
                ...
                if observations.get(f"observation.depth.{cam}") is None:
                    logging.warning(
            ...
            with runtime.no_sync(policy, accum_idx, grad_accum):
                ...
                if depth_on and capture_modality_telemetry and accum_idx == 0:
                    ...
                    if late is not None and raw_policy._depth_token_rms is not None:
                        accum["depth_projected_late_rms_ratio"] = raw_policy._depth_token_rms.item() / max(
                            late.item(), 1e-12
                        )
            ...
            accum["loss_depth_event"] += float(metrics.get("depth_gripper_event_loss", 0.0)) / grad_accum
            accum["depth_event/close_bce"] += float(metrics.get("depth_gripper_close_bce", 0.0)) / grad_accum
            accum["depth_event/open_bce"] += float(metrics.get("depth_gripper_open_bce", 0.0)) / grad_accum

            actor_loss_raw = metrics.get(
                "loss_raw", loss.detach().float() if isinstance(loss, torch.Tensor) else None
            )
        ...
        if depth_on and capture_modality_telemetry:
            ...
            grads_by_component: dict[str, list[torch.Tensor]] = {component: [] for component in components}
        ...
        if flow_loss_per_sample_list:
            ...
            accum["flow_loss_per_sample_std"] = (
                all_flow_per_sample.std().item() if all_flow_per_sample.numel() > 1 else 0.0
            )
        ...
        if auxiliary_loss_list:
            accum["auxiliary_loss_per_sample_histogram"] = torch.cat(auxiliary_loss_list).cpu().numpy()
    ...
    def _subtask_generation_loss(
        ...
        try:
            ...
            step.prompt_mode = "action"
        # use_cache must be explicit: the HF composite config has no use_cache attribute,
        # so the forward's config fallback raises AttributeError. The base model (no lm_head)
        # is called directly and the lm_head applied only at answer positions: full-sequence
        # logits are [B, T, vocab] and dominate VRAM, yet most of T is image tokens that
        # never carry labels.
        hidden = policy.model.model(**policy._model_inputs(batch), use_cache=False).last_hidden_state
        labels = batch["labels"]
    ...
    def log_metrics(
        ...
        aim_infos = self._aim_metrics(training_infos)
