...
"""Offline RLT actor/critic trainer for tinypi05.

Mirrors `train_pi05_rlt_head_offline.py`, swapping in the tinypi05 RLT policy.
The persisted `RLTReplayBuffer` format is policy-agnostic: it stores the
already-encoded `rl_token`, proprio, reference_chunk, executed_chunk and
...
@dataclass
class TrainTinyPI05RLTHeadOfflineConfig:
    policy_path: str = (
    ...
    output_dir: str = "outputs/tinypi05_rlt_offline_head"
    device: str = "cuda"
    seed: int = 0
...
def _build_policy(
    cfg: TrainTinyPI05RLTHeadOfflineConfig, replay: RLTReplayBuffer
) -> TinyPI05RLTPolicy:
...
def _init_wandb(cfg: TrainTinyPI05RLTHeadOfflineConfig):
...
def _save_head(
    ...
    output_dir: Path,
    step: int,
    cfg: TrainTinyPI05RLTHeadOfflineConfig,
) -> None:
    ...
    logging.info("Saved offline tinypi05 RLT head checkpoint to %s", path)


@draccus.wrap()
def train_tinypi05_rlt_head_offline(cfg: TrainTinyPI05RLTHeadOfflineConfig) -> None:
    init_logging()
    _set_seed(cfg.seed)
    ...
    logging.info(
        "Starting offline tinypi05 RLT head training | replay=%s | samples=%d | device=%s | steps=%d",
...
if __name__ == "__main__":
    train_tinypi05_rlt_head_offline()
