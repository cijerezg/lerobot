...
This is the Jacobian variant of :mod:`lerobot.probes.attention_spatial`.
...
from lerobot.probes.attention_spatial import run_jacobian
...
@dataclass
class ProbeSpatialMemorizationJacobianConfig(TrainRLServerPipelineConfig):
...
@parser.wrap()
def probe_cli(cfg: ProbeSpatialMemorizationJacobianConfig):
    init_logging()
    p = cfg.probe_parameters
    device = get_safe_torch_device(try_device=cfg.policy.device)
    output_dir = os.path.join(p.output_dir, "spatial_memorization_jacobian")
