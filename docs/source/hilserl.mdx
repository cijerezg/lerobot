...
3. **Safety & efficiency tools:** joint/end-effector (EE) bounds, crop region of interest (ROI) preprocessing and Aim monitoring keep the data useful and the hardware safe.
...
- A successful experiment is one where the human has to intervene at the start but then reduces the amount of interventions as the policy improves. You can monitor the intervention rate in the `Aim` dashboard.
...
If you have `aim.enable` set to `true` in your configuration, you can monitor training progress in real-time in the local Aim dashboard (run aim up --repo ./aim).
