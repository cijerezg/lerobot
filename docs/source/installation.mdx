...
| `training` | `dataset` + `accelerate`, `aim`           | Training policies                   |
...
Aim is included in the `training` extra. It stores runs locally and does not require an account. Aim logging currently requires Python 3.12. Enable it with `--aim.enable=true`, then launch the web UI with:

```bash
aim up --repo ./aim
```

Open `http://127.0.0.1:43800` to inspect and compare runs.
