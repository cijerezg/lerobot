...
| Old non-RTC inference workers built `torch.tensor([[cfg.policy.inference_advantage]])` unconditionally | Historical pi05-era actor/critic-logging paths; would **crash** on `inference_advantage: null` | Resolved by deletion of the unreachable non-RTC workers during the 2026-09-24 cleanup; RTC helpers remain |
