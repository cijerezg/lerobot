...
| `mem_history_regime` | *which frames* — where does history help, where does it hurt, and is either real? | $z$ on $\mathrm{MSE}(\mathrm{stale})-\mathrm{MSE}(\mathrm{full})$, the paired test against the same window taken $W$ s too early: below 2 the model reacted to *a* window rather than reading *this* one. Then helped/hurt fractions against $\tau=Q_{0.9}(\|\Delta\|)$ under a reseed alone, and Spearman $\rho$ of the signed per-frame $\Delta$ against episode phase, lookback travel, scene change, chunk travel, gripper swing, baseline error and repeat-padding. Exists because the usefulness bar above is a mean over a population that is half positive: on v6/1600, $+0.00067$ out of 47% helped and 53% hurt |
